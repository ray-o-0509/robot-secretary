...
export function TasksView({ payload }: Props) {
  ...
  if (payload.error) {
    return <ErrorState message={payload.error} hint={t('tasks.ticktickError')} />
  }
      ...
      {todos.map((t) => (
        <Card key={t.taskId} accent={t.priority === 'high' ? 'magenta' : 'cyan'}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            <Checkbox onClick={() => handleCompleteTask(t.taskId, t.projectId)} label={t('tasks.complete')} />
...
function Checkbox({ onClick, label }: { onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      title={label}
