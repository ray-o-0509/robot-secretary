...
export function CalendarView({ payload }: Props) {
  ...
  if (payload.type === 'calendar_week') {
    ...
    const groups = groupByDate(data.events, weekdays)
    return (
      <>
        {errors.map((a) => (
          <ErrorState key={a.account} message={`${a.account}: ${a.error}`} />
...
function EventCard({ event }: { event: CalendarData['events'][number] }) {
  ...
  return (
    <Card accent="cyan">
      ...
      >
        {formatTime(event, t('calendar.allDay'))}
      </div>
      {event.location && (
        <div
          style={{
...
function formatTime(e: CalendarData['events'][number], allDayLabel: string): string {
...
function groupByDate(events: CalendarData['events'], weekdays: string[]) {
...
function emptyMessage(type: PanelPayload['type'], t: (key: string) => string): string {
  if (type === 'calendar_today') return t('calendar.emptyToday')
  if (type === 'calendar_tomorrow') return t('calendar.emptyTomorrow')
