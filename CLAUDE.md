...
This is an **Electron + React + Three.js** desktop app: a transparent, always-on-top, click-through window containing a floating 3D robot that runs as a Gemini Live voice assistant with function-calling into Slack / Gmail / Google Calendar / Notion.
...
- **Gmail and Calendar share one OAuth token file** at `~/.robot-secretary-gmail-token.json` (defined in both files). There is **no in-app OAuth flow** — the token must be created externally and dropped at that path; both tools just read it. If you add new Google scopes, the existing token will be missing them.
- **Notion expects a database with `Name` (title), `Status` (status), and `Due Date` (date) properties.** Other schemas will silently return empty fields.
- **Slack `getUnreadMessages` without a channel iterates every joined channel** to check `unread_count` — this can be slow for users in many workspaces.

### Configuration: split between two stores

Two separate configuration mechanisms exist and they are not unified:

- **`.env` / `.env.local`** — loaded by the main process at startup (`dotenv.config` against `__dirname/../.env*`). Tool modules in `src/main/tools/*` read from `process.env`. This is where Slack / Notion / Google credentials must live for tool calls to work.
- **`localStorage`** in the renderer — written by `SettingsPanel.tsx`. **Only `GEMINI_API_KEY` is actually consumed** (by `useGeminiLive`, which falls back to `import.meta.env.VITE_GEMINI_API_KEY`). The Slack/Notion fields in the settings panel are written but never read — main-process tools only see `process.env`. Treat this as a known gap when touching settings: if you wire a key through the UI, you must also propagate it to the main process (e.g. via IPC) or the tool calls will keep using `.env`.
