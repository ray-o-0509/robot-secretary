...
- **TickTick** uses an access token at `~/.claude/skills/todos/.ticktick.json` (`{"access_token": "..."}`) read by `src/main/tools/tickTickAuth.ts`. Override with `TICKTICK_ACCESS_TOKEN` env. Token is shared with the user's `daily-dashboard` Vercel project; if it expires, pull from there with `vercel env pull` against `daily-viewer`.
...
- **`.env` / `.env.local`** — loaded by the main process at startup (`dotenv.config` against `__dirname/../.env*`). Tool modules in `src/main/tools/*` read from `process.env`. This is where API-key style credentials (Gemini / Anthropic / Slack / Turso) live; Google and TickTick instead read from external token files (see Tool modules above).
